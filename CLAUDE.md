...
UI 카피(화면에 보이는 버튼/라벨/placeholder/토스트 메시지 등)는 영어가 기본이다. 코드 주석, README, 커밋 메시지는 한국어를 유지한다. 새 UI 텍스트를 추가할 때도 영어로 작성한다. `title`/`aria-label` 같은 접근성·툴팁 속성도 영어로 통일한다 — 단, 보드 제목·사용자 이름처럼 사용자가 직접 입력한 데이터는 원래 값 그대로 둔다.
